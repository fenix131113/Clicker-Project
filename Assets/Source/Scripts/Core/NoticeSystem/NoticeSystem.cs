...
public class NoticeSystem
{
    ...
    {
        GameObject notification = MonoBehaviour.Instantiate(objectsContainer.NoticePanelPrefab, objectsContainer.NotificationsContainer);
        ...
        notification.GetComponent<RectTransform>().DOMoveY(notification.GetComponent<RectTransform>().position.y + notification.GetComponent<RectTransform>().sizeDelta.y, 0.5f).onComplete += () =>
        {
            ...
            deleteNotificationAnim.onComplete += () => MonoBehaviour.Destroy(notification);
        };
    }
}
